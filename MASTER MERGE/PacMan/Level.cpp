...
void Level::draw( ID3DX10Sprite * p_spriteBatch, ID3D10DepthStencilView* p_depthView,ID3D10RenderTargetView* p_renderTarget,D3D10_VIEWPORT p_VP)
{
	...
	if (debug)
	{
		...
		for each(Object* o in m_objects.m_enemies)
		{
			o->render(m_camera->getViewMatrix(), m_camera->getProjectionMatrix());
		}
...
void Level::update( float p_deltaTime )
{
	...
	//for each(Object* o in m_objects.m_walls)
	//{
	//	if ( m_objects.m_pacman->getBoundingBox()->boxVsBox(o->getBoundingBox()) == 1 )
	//	{
	//		//KROCK
	//	}
	//}
	
	for(UINT i = 0; i <  m_objects.m_cherries.size(); i++)
	{
		if ( m_objects.m_pacman->getBoundingBox()->boxVsBox(m_objects.m_cherries.at(i)->getBoundingBox()) == 1 )
		{
			m_objects.m_cherries.erase(m_objects.m_cherries.begin() + i);
			m_player->updateScore(200);
		}
	}

	for(UINT i = 0; i <  m_objects.m_candies.size(); i++)
	{
		if ( m_objects.m_pacman->getBoundingBox()->boxVsBox(m_objects.m_candies.at(i)->getBoundingBox()) == 1 )
		{
			m_objects.m_candies.erase(m_objects.m_candies.begin() + i);
			m_player->updateScore(10);
		}
	}
	
	for(UINT i = 0; i <  m_objects.m_powerUps.size(); i++)
	{
		if ( m_objects.m_pacman->getBoundingBox()->boxVsBox(m_objects.m_powerUps.at(i)->getBoundingBox()) == 1 )
		{
			m_objects.m_powerUps.erase(m_objects.m_powerUps.begin() + i);
			m_player->updateScore(10);
		}
	}

	for(UINT i = 0; i <  m_objects.m_enemies.size(); i++)
	{
		if ( m_objects.m_pacman->getBoundingBox()->boxVsBox(m_objects.m_enemies.at(i)->getBoundingBox()) == 1 )
		{
			//m_objects.m_powerUps.erase(m_objects.m_powerUps.begin() + i);
			//m_player->updateScore(10);
			exit(0);
