...
HRESULT Mesh::Initialize(ID3D10Device* p_d3dDevice)
{
	...
	//Create Shader
	m_shader = new Shader();
	if(FAILED(m_shader->Init(m_d3dDevice, "Line.fx", lineVertexLayout, sizeof(lineVertexLayout) / sizeof(D3D10_INPUT_ELEMENT_DESC), "DrawLine",
...
void Mesh::PrepareToRender(D3DXMATRIX& p_mMVP, D3DXMATRIX& p_mWIT)
{
	...
	m_shader->SetMatrix("g_mWorldIT", p_mWIT);
...
void Mesh::RenderSpecificShader(Shader* pShader, D3DXMATRIX& p_mLMVP)
{
	m_d3dDevice->IASetPrimitiveTopology( D3D10_PRIMITIVE_TOPOLOGY_TRIANGLELIST);
	m_vertexBuffer->Apply(0);
