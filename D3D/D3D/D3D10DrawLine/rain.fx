...
// Modifyed by Philip Andersson 2012
//=============================================================================
//***********************************************
// GLOBALS                                      *
//***********************************************
	...
	{	
		...
		if( gIn[0].age > 0.001f )
		{
			for(int i = 0; i < 5; ++i)
			{
				...
				float3 vRandom = 512.0f*RandVec3((float)i/20.0f);
				vRandom.y = 20.0f;
			
				Particle p;
				p.initialPosW = gEmitPosW.xyz + vRandom;
				p.initialVelW = float3(0.0f, 0.0f, 0.0f);
				p.sizeW       = float2(20.0f, 20.0f);
				p.age         = 0.0f;
				p.type        = PT_FLARE;
...
{
	return float4(0.0f, 0.6f, 0.8f, 0.5f);//gTexArray.Sample(gTriLinearSam, float3(pIn.texC, 0));
}
