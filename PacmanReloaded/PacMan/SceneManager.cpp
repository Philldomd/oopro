...
void SceneManager::drawScene(ID3D10DepthStencilView* p_depthView,ID3D10RenderTargetView* p_renderTarget,D3D10_VIEWPORT p_VP)
{
	...
	m_primaryScene->draw(m_spriteBatch, p_depthView, p_renderTarget, p_VP);
	if(m_secondaryScene)
		m_secondaryScene->draw(m_spriteBatch, p_depthView, p_renderTarget, p_VP);
	//m_spriteBatch->Flush();
	m_spriteBatch->End();
}
